...
API_URL = 'http://localhost/flyers'

def format_url_query(params: dict) -> str:
...
def test_cant_create_flyer_without_authentication():
    ...
    r = requests.put(API_URL, json = flyer) 
...
def test_cant_create_flyer_with_partial_data():
    ...
    r = requests.put(API_URL, json = partial_flyer, auth = restaurant_auth) 
...
def populate_menus(menus):
    for item in menus:
        auth, menu = item
        r = requests.put(API_URL, json = menu, auth=auth) 
        assert r.status_code == 200
...
def test_todays_menus_finds_all_todays_menus():
    ...
    r = requests.get(format_url_query(dict(date=today)))
    assert r.status_code == 200
    query_result = r.json()
    assert len(query_result) == 3
